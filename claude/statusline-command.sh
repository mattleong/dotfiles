...
if [[ -n "$usage_data" ]] && jq -e . >/dev/null 2>&1 <<< "$usage_data"; then
    bar_width=10
    col1w=23
    col2w=22

    # ---- 5-hour (current) ----
    five_hour_pct=$(jq -r '.five_hour.utilization // 0' <<< "$usage_data" | awk '{printf "%.0f", $1}')
    five_hour_reset_iso=$(jq -r '.five_hour.resets_at // empty' <<< "$usage_data")
    ...
    # ---- 7-day (weekly) ----
    seven_day_pct=$(jq -r '.seven_day.utilization // 0' <<< "$usage_data" | awk '{printf "%.0f", $1}')
    seven_day_reset_iso=$(jq -r '.seven_day.resets_at // empty' <<< "$usage_data")
    ...
    col3_bar=""
    col3_reset=""
    extra_enabled=$(jq -r '.extra_usage.is_enabled // false' <<< "$usage_data")
    if [[ "$extra_enabled" == "true" ]]; then
        extra_pct=$(jq -r '.extra_usage.utilization // 0' <<< "$usage_data" | awk '{printf "%.0f", $1}')
        extra_used=$(jq -r '.extra_usage.used_credits // 0' <<< "$usage_data" | awk '{printf "%.2f", $1/100}')
        extra_limit=$(jq -r '.extra_usage.monthly_limit // 0' <<< "$usage_data" | awk '{printf "%.2f", $1/100}')
